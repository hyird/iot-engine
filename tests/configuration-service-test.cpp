...
int main() {
    try {
        requireNoUnsafeJsonCasts(configurationServiceSource());
        std::cout << "configuration service tests passed\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "configuration service test failed: " << error.what() << '\n';
        return 1;
    }
}
