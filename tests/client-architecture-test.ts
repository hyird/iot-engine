...
const native = join(root, 'clients/windows/native');
const walk = (dir: string): string[] => readdirSync(dir).flatMap(name => {
    const file = join(dir, name);
    return statSync(file).isDirectory() ? (name === 'vendor' ? [] : walk(file)) : [file];
});
const files = [...walk(native), ...walk(join(root, 'clients/windows/winui'))].filter(f => /\.(h|cpp)$/.test(f));
...
for (const file of files) {
    ...
    for (const match of readFileSync(file, 'utf8').matchAll(/^\s*#include\s+"([^"]+)"/gm)) {
        const target = [resolve(dirname(file), match[1]), resolve(native, 'common', match[1])].find(p => paths.has(p));
...
test('Windows UI and shared primitives never depend on the service implementation', () => {
    ...
    for (const file of files) {
        if (!/\/(common|gui|winui)\//.test(label(file))) continue;
        const visited = new Set<string>();
        const visit = (node: string) => {
            if (visited.has(node)) return;
            visited.add(node);
            if (label(node).includes('/native/service/')) violations.push(`${label(file)} -> ${label(node)}`);
