...
public:
    explicit WinHttpPlatformVpnApi(std::wstring host = L"i.a-z.xin", unsigned short port = 443, bool tls = true);
    ~WinHttpPlatformVpnApi();
...
private:
    Json request(std::string_view method, std::string_view path, std::string_view token, const Json* body, std::stop_token stop);
    std::stop_token sessionToken();
    std::mutex connectionGate_;
    std::stop_source sessionStop_;
    std::wstring host_;
...
private:
    std::string line_, data_, event_ = "message";
};
