...
void VpnConnectionService::run(std::stop_token stop) {
    unsigned failures = 0;
    std::optional<std::uint64_t> retryGeneration;
    while (!stop.stop_requested()) {
        ...
        if (retryGeneration != generation) { failures = 0; retryGeneration = generation; }
        ...
        for (;;) {
            try {
                api_->watchConfig(accessToken, peer, [&](const Json& config) {
                    ...
                    acceptConfig(config, watch->get_token()); receivedSnapshot = true;
                }, watch->get_token());
        ...
        if (generation != generation_ || watch->stop_requested() || !shouldWatch()) continue;
        // An initial snapshot followed by an immediate disconnect must not reset the retry limit.
        if (receivedSnapshot && std::chrono::steady_clock::now() - connectedAt >= std::chrono::seconds(30)) failures = 1;
        if (failures >= 9) {
            tunnel_->stop();
            setStatus("Error", "配置订阅多次重连失败，请手动连接或同步后重试。");
            changes_.wait(lock, stop, [&] { return generation_ != generation || !shouldWatch(); });
            continue;
        }
...
void VpnConnectionService::stop() {
    std::lock_guard lock(gate_); invalidateWatch(); api_->closeSession(); tunnel_->stop(); setStatus("Stopped");
}
}
