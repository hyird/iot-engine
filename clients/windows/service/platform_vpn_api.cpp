...
public:
    ~AsyncRequest() { close(); }
    void open(const std::wstring& host, unsigned short port, bool tls, std::string_view method, std::string_view path, std::string_view token, bool stream) {
        ...
        connection_.value = WinHttpConnect(session_.value, host.c_str(), port, 0);
        if (!connection_.value) win32Error("Cannot connect to platform");
        const auto verb = utf16(method), resource = utf16(path);
        request_ = WinHttpOpenRequest(connection_.value, verb.c_str(), resource.c_str(), nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, tls ? WINHTTP_FLAG_SECURE : 0);
...
Json envelope(std::string_view text, int status, bool errorEvent = false) {
    ...
    if (status < 200 || status >= 300 || code != 0 || errorEvent) {
        ...
        throw PlatformVpnApiError(message, status >= 200 && status < 300 ? statusForCode(code) : status, code);
    }
...
Json requestJson(const std::wstring& host, unsigned short port, bool tls, std::string_view method, std::string_view path, std::string_view token, const Json* body, std::stop_token stop) {
    AsyncRequest request; request.open(host, port, tls, method, path, token, false); request.send(body, stop);
    return envelope(readBody(request, stop), static_cast<int>(request.status()));
}
void stream(const std::wstring& host, unsigned short port, bool tls, std::string_view path, std::string_view token, const std::function<bool(const Json&)>& receive, std::stop_token stop) {
    AsyncRequest request; request.open(host, port, tls, "GET", path, token, true); request.send(nullptr, stop);
...
WinHttpPlatformVpnApi::WinHttpPlatformVpnApi(std::wstring host, unsigned short port, bool tls)
    : host_(std::move(host)), port_(port), tls_(tls) {}
WinHttpPlatformVpnApi::~WinHttpPlatformVpnApi() { closeSession(); }
void WinHttpPlatformVpnApi::closeSession() noexcept {
    std::stop_source previous;
    { std::lock_guard lock(connectionGate_); previous = sessionStop_; sessionStop_ = std::stop_source{}; }
    previous.request_stop();
}
std::stop_token WinHttpPlatformVpnApi::sessionToken() {
    std::lock_guard lock(connectionGate_); return sessionStop_.get_token();
}
Json WinHttpPlatformVpnApi::request(std::string_view method, std::string_view path, std::string_view token, const Json* body, std::stop_token stop) {
    std::stop_source cancellation;
    std::stop_callback callerCancelled(stop, [&] { cancellation.request_stop(); });
    std::stop_callback sessionClosed(sessionToken(), [&] { cancellation.request_stop(); });
    return requestJson(host_, port_, tls_, method, path, token, body, cancellation.get_token());
}
Json WinHttpPlatformVpnApi::login(std::string_view user, std::string_view password, std::stop_token stop) {
    const Json body{{"username", user}, {"password", password}}; return request("POST", "/v1/auth/login", {}, &body, stop);
}
Json WinHttpPlatformVpnApi::refresh(std::string_view token, std::stop_token stop) {
    const Json body{{"refresh_token", token}}; return request("POST", "/v1/auth/refresh", {}, &body, stop);
}
Json WinHttpPlatformVpnApi::devices(std::string_view token, std::stop_token stop) {
    auto result = request("GET", "/v1/vpn/desktop/devices", token, nullptr, stop);
    if (!result.is_array()) throw std::runtime_error("平台没有返回有效设备列表。"); return result;
}
Json WinHttpPlatformVpnApi::apply(std::string_view token, std::string_view peer, const Json& body, std::stop_token stop) {
    return request(peer.empty() ? "POST" : "PATCH", peer.empty() ? "/v1/vpn/desktop/peers" : peerPath(peer), token, &body, stop);
}
Json WinHttpPlatformVpnApi::config(std::string_view token, std::string_view peer, std::stop_token stop) {
    auto result = request("GET", peerPath(peer) + "/config", token, nullptr, stop);
    if (!result.is_object()) throw std::runtime_error("平台没有返回有效配置。"); return result;
}
void WinHttpPlatformVpnApi::remove(std::string_view token, std::string_view peer, std::stop_token stop) { request("DELETE", peerPath(peer), token, nullptr, stop); }
void WinHttpPlatformVpnApi::watchConfig(std::string_view token, std::string_view peer, const std::function<void(const Json&)>& receive, std::stop_token stop) {
    std::stop_source cancellation;
    std::stop_callback callerCancelled(stop, [&] { cancellation.request_stop(); });
    std::stop_callback sessionClosed(sessionToken(), [&] { cancellation.request_stop(); });
    try { stream(host_, port_, tls_, peerPath(peer) + "/config/events", token, [&](const Json& snapshot) {
        if(cancellation.stop_requested()) return false;
        receive(snapshot); return !cancellation.stop_requested();
    }, cancellation.get_token()); } catch (...) { if (!cancellation.stop_requested()) throw; }
}
}
