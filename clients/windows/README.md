...
从桌面或开始菜单启动，登录后选择设备并应用。关闭窗口不停止后台连接。本机服务未安装、未启动或管道不可用时，界面显示「修复服务」；确认管理员权限后重新注册并启动 `iot-egine.tunnel`，不必关闭窗口或重跑安装包。登录、设备查询与写操作使用 WinHTTP HTTP/JSON，配置变化通过 SSE 推送，不轮询业务数据。取消订阅不影响其他请求；注销或停止服务时取消会话内仍在进行的请求与订阅。
