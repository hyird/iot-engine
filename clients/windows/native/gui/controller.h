...
struct Device { std::string id, name, imei, subnet, address; bool online = false; };
class Controller {
public:
    using Transport = std::function<Json(const Json&, std::stop_token)>;
    using Clock = std::function<std::chrono::steady_clock::time_point()>;
    explicit Controller(Transport transport, bool synchronous = false, Clock clock = [] { return std::chrono::steady_clock::now(); })
        : transport_(std::move(transport)), synchronous_(synchronous), clock_(std::move(clock)) {}
    ~Controller() { worker_.request_stop(); if (worker_.joinable()) worker_.join(); }
    ...
    std::vector<Device> filtered(std::string_view query) const {
        const auto lower = [](std::string value) { for (auto& c : value) if (c >= 'A' && c <= 'Z') c += 'a' - 'A'; return value; };
        const auto needle = lower(std::string(query)); std::vector<Device> result;
...
private:
    ...
    void loadDevices(const Json& snapshot) {
        ...
        std::vector<Device> next; std::set<std::string> seen;
