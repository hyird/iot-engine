...
try{auto t=createWindowsWireGuardTunnel();std::set<std::string> keys;for(int i=0;i<16;++i){auto k=t->generateKeys();ok(k.first.size()==44&&k.second.size()==44,"key size");keys.insert(k.first);keys.insert(k.second);}ok(keys.size()==32,"key uniqueness");std::cout<<"PASS native keys\n";}catch(const std::exception&e){++bad;std::cout<<"FAIL native keys: "<<e.what()<<"\n";}
