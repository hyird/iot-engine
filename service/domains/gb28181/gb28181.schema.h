...
class GbNameValidator final : public ruvia::Middleware<GbNameValidator> {
public:
  RUVIA_VALIDATE_JSON(GbNameBody,
                      RUVIA_RULE(name, RUVIA_REQUIRED("名称不能为空"),
                                 RUVIA_MAX(255, "名称不能超过 255 个字符")));
};

inline std::string requiredName(const GbNameBody &body) {
  const auto &input = body.get<"name">();
  if (!input)
    service::common::fail(10001, "名称不能为空", 400);
...
inline double finiteQuery(ruvia::Context &c, std::string_view name,
