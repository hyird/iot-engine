...
namespace service::utils::network {
...
inline std::optional<Ipv4Cidr> parseCidr(std::string_view input,
    ...
    const auto [end, error] =
        std::from_chars(prefixText.data(), prefixText.data() + prefixText.size(), prefix);
    if (prefixText.empty() || error != std::errc{} ||
        end != prefixText.data() + prefixText.size() || prefix < minPrefix ||
        prefix > maxPrefix)
        return std::nullopt;
...
inline bool isPrivateIpv4(const Ipv4Cidr& cidr) noexcept {
    ...
    return private10.contains(cidr.network) &&
               private10.contains(cidr.network + cidr.size() - 1U) ||
           private172.contains(cidr.network) &&
               private172.contains(cidr.network + cidr.size() - 1U) ||
           private192.contains(cidr.network) &&
               private192.contains(cidr.network + cidr.size() - 1U);
}
