...
class UserService {
  public:
    ...
    ruvia::Task<UserPageDataDto> list(ruvia::Context &c, std::int64_t page, std::int64_t pageSize,
        ...
        const auto rows = co_await query.getMany<UserDetails>();
    ...
    ruvia::Task<ruvia::BoxedArray<UserOptionDto>> options(ruvia::Context &c,
        ...
        options.take = 100;
        const auto rows = co_await c.db().getRepository<UserEntity>().find(options);
        ...
        for (const auto &row : rows) {
            ...
            item.set<"id">(row.get<"id">())
                .set<"username">(row.get<"username">())
                .set<"nickname">(row.isNull<"nickname">() ? std::string_view{} : row.get<"nickname">());
        }
        co_return result;
    }
    ...
    ruvia::Task<void> create(ruvia::Context &c, const CreateUserBody &body) {
        ...
        (void)co_await tx.getRepository<UserEntity>().insert(user);
    ...
    ruvia::Task<void> update(ruvia::Context &c, std::string_view id, const UpdateUserBody &body) {
        ...
        const auto existing = co_await c.db().getRepository<UserEntity>().findOne(options);
        if (!existing)
            service::common::fail(12001, "用户不存在", 404);
        const std::string username(existing->get<"username">());
        ...
        if (changed) {
            ...
            (void)co_await tx.getRepository<UserEntity>().update(UserEntity::column<"id">() == id, changes);
        }
        if (body.get<"roleIds">())
            co_await replaceRoles(tx, id, *body.get<"roleIds">(), c.pool());
        co_await tx.commit();
    ...
    ruvia::Task<void> remove(ruvia::Context &c, std::string_view id, std::string_view operatorId) {
        ...
        ruvia::DbFindOptions options;
        options.where = db::activeId<UserEntity>(id);
        const auto user = co_await c.db().getRepository<UserEntity>().findOne(options);
        if (!user)
            service::common::fail(12001, "用户不存在", 404);
        if (user->get<"username">() == "admin")
            service::common::fail(12004, "内置管理员不能删除", 400);
        ruvia::DbExpressions expressions(c.pool());
        (void)co_await c.db().getRepository<UserEntity>().update(
            UserEntity::column<"id">() == id,
            {{"deleted_at", expressions.call("now")}, {"updated_at", expressions.call("now")}});
    }

  private:
    static ruvia::DbQueryBuilder<UserEntity, ruvia::DbHandle> userSelect(ruvia::Context& c) {
        using namespace ruvia;
        DbExpressions expressions(c.pool());
        auto query = c.db().getRepository<UserEntity>().createQueryBuilder("u");
        query.select({
            {"id", expressions.column("id", "u")},
            {"username", expressions.column("username", "u")},
            {"nickname", expressions.coalesce({expressions.cast(expressions.column("nickname", "u"), DbDataType::kText), expressions.value("")})},
            {"phone", expressions.coalesce({expressions.cast(expressions.column("phone", "u"), DbDataType::kText), expressions.value("")})},
            {"email", expressions.coalesce({expressions.cast(expressions.column("email", "u"), DbDataType::kText), expressions.value("")})},
            {"status", expressions.column("status", "u")},
            {"department_id", expressions.coalesce({expressions.cast(expressions.column("department_id", "u"), DbDataType::kText), expressions.value("")})},
            {"department_name", expressions.coalesce({expressions.cast(expressions.column("name", "d"), DbDataType::kText), expressions.value("")})},
            {"created_at", expressions.call("iot_utc_timestamp", {expressions.column("created_at", "u")})},
            {"updated_at", expressions.call("iot_utc_timestamp", {expressions.column("updated_at", "u")})}});
        query.join<service::dept::DeptEntity>(DbJoinType::kLeft, "d",
            expressions.binary(expressions.column("id", "d"), DbBinaryOperator::kEqual,
                               expressions.column("department_id", "u")));
        return query;
    }

    static void fillBase(UserItemDto &item, const UserDetails &row) {
        item.set<"id">(row.get<"id">());
        item.set<"username">(row.get<"username">());
        item.set<"nickname">(row.get<"nickname">());
        item.set<"phone">(row.get<"phone">());
        item.set<"email">(row.get<"email">());
        item.set<"status">(row.get<"status">());
        item.set<"departmentId">(row.get<"department_id">());
        item.set<"departmentName">(row.get<"department_name">());
    ...
    loadRoles(ruvia::Context &c, std::string_view userId) {
        ...
        query.select({{"id"}, {"name"}, {"code"}})
            .join<UserRoleEntity>(ruvia::DbJoinType::kInner, "ur",
