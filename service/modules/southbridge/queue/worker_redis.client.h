...
// one file. Every South Worker owns exactly one adapter and its Redis pool size is
// forced to one connection.
class WorkerRedisClient final {
  public:
    WorkerRedisClient(asio::io_context& ioContext, ruvia::RedisConfig config,
                      WorkerTimerScheduler& scheduler)
        : resource_(), scheduler_(scheduler), pool_(ioContext, oneConnection(std::move(config)),
                                                       &resource_) {}

    WorkerRedisClient(const WorkerRedisClient&) = delete;
    WorkerRedisClient& operator=(const WorkerRedisClient&) = delete;

    ~WorkerRedisClient() { close(); }
  ...
  private:
    ...
    WorkerTimerScheduler::Token deadlineScanToken_ = 0;
    bool closed_ = false;
};
