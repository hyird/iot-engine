...
namespace service::southbridge::config_redis {

namespace redis_async = service::bridge::redis_async;
...
// A deterministic content signature makes periodic Northbridge reconciliation cheap and
...
inline std::string stringReply(const ruvia::RedisValue& value, std::string_view operation) {
    if (value.kind() != ruvia::RedisValue::Kind::kString)
        redis_async::throwValue(operation, value);
...
inline std::vector<bridge::StreamField> hashFields(const ruvia::RedisValue& value,
                                                   std::string_view operation) {
    if (value.kind() != ruvia::RedisValue::Kind::kArray)
        redis_async::throwValue(operation, value);
    ...
    if (values.size() % 2 != 0)
        redis_async::throwValue(operation, value);
    std::vector<bridge::StreamField> result;
    ...
    if constexpr (requires { redis.pipeline(); }) {
        ...
        for (const auto& reply : replies)
            if (reply.kind() == ruvia::RedisValue::Kind::kError)
                redis_async::throwValue("runtime config pipeline", reply);
        co_return replies;
    } else {
...
return replies
        ...
        for (const auto& command : commands) {
            if (command.size() != 2 || (command[0] != "HGETALL" && command[0] != "SMEMBERS"))
                throw std::runtime_error("unsupported runtime config batch command");
        ...
        if (reply.kind() != ruvia::RedisValue::Kind::kArray)
            redis_async::throwValue("runtime config batch", reply);
...
inline void appendHash(std::vector<std::vector<std::string>>& commands, std::string_view version,
                       std::string key, const std::vector<bridge::StreamField>& fields) {
...
inline std::int64_t integer(const std::vector<bridge::StreamField>& fields, std::string_view name,
    ...
    if (error != std::errc{} || end != value.data() + value.size())
        throw std::runtime_error("invalid runtime config integer: " + std::string(name));
    return result;
}

inline double decimal(const std::vector<bridge::StreamField>& fields, std::string_view name,
                      double fallback = 0) {
    const auto value = field(fields, name);
    return value.empty() ? fallback : std::stod(std::string(value));
}

inline DeviceDefinition device(const std::vector<bridge::StreamField>& fields) {
    ...
    result.heartbeatBytes = bridge::fromHex(field(fields, "heartbeat_hex"));
    result.registrationMode = field(fields, "registration_mode");
    result.registrationBytes = bridge::fromHex(field(fields, "registration_hex"));
...
inline ElementDefinition element(const std::vector<bridge::StreamField>& fields) {
...
ruvia::Task<std::vector<std::string>> members(const Redis& redis, std::string key) {
    co_return stringArray(co_await redis_async::command(redis, {"SMEMBERS", std::move(key)}),
                          "SMEMBERS runtime config");
}
...
ruvia::Task<void> eraseSnapshot(const Redis& redis, std::string_view version) {
    ...
    for (std::size_t offset = 0; offset < keys.size(); offset += batchSize) {
        ...
        (void)co_await redis_async::command(redis, args);
    }
}

} // namespace detail

// Northbridge projection. The version pointer is switched only after every readable
// Hash/Set has been written, so South Workers never observe a partial snapshot.
template <typename Redis>
ruvia::Task<std::string> project(const Redis& redis, const RuntimeSnapshot& snapshot) {
    ...
    const auto activeReply =
        co_await redis_async::command(redis, {"GET", std::string(kActiveVersionKey)});
    if (!activeReply.null()) {
        const auto active = detail::stringReply(activeReply, "GET active runtime config");
        const auto signatureReply =
            co_await redis_async::command(redis, {"HGET", metadataKey(active), "signature"});
        if (!signatureReply.null() &&
            detail::stringReply(signatureReply, "HGET runtime config signature") ==
                snapshotSignature)
            co_return active;
    }

    const auto version = bridge::nextMessageId();
    const auto createdAt = bridge::utcNowMilliseconds();
    (void)co_await redis_async::command(
    ...
    for (const auto& device : snapshot.devices) {
        ...
        detail::appendHash(
            ...
            {{"id", device.id},
             ...
             {"heartbeat_hex", bridge::toHex(device.heartbeatBytes)},
             {"registration_mode", device.registrationMode},
             {"registration_hex", bridge::toHex(device.registrationBytes)},
    ...
    if (!previousReply.null()) {
        const auto previous = detail::stringReply(previousReply, "GET active runtime config");
        if (!previous.empty() && previous != version)
            (void)co_await redis_async::command(redis, {"ZADD", std::string(kVersionsKey), "NX",
                                                        std::to_string(createdAt), previous});
    }
    (void)co_await redis_async::command(redis, {"SET", std::string(kActiveVersionKey), version});
...
template <typename Redis> ruvia::Task<std::string> activeVersion(const Redis& redis) {
    ...
    if (versionReply.null())
        throw std::runtime_error("runtime config is not ready");
    co_return detail::stringReply(versionReply, "GET active runtime config");
}
...
ruvia::Task<RuntimeSnapshot> load(const Redis& redis, std::string version) {
    ...
    for (const auto& linkId : co_await detail::members(redis, prefix(version) + ":links")) {
        const auto fields = co_await redis_async::hashEntries(redis, linkKey(version, linkId));
        LinkDefinition link;
        ...
        for (const auto& targetId :
            ...
            const auto targetFields =
                co_await redis_async::hashEntries(redis, targetKey(version, linkId, targetId));
