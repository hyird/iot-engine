...
#include "service/modules/southbridge/southbridge.worker.h"

namespace service::southbridge {

class SouthbridgeRuntime final {
  public:
    SouthbridgeRuntime() = default;
    ...
    void start(ruvia::RedisConfig redisConfig, std::size_t workerCount) {
        ...
        try {
            ...
            for (std::size_t index = 0; index < workerCount; ++index)
                workers_.push_back(std::make_unique<SouthbridgeWorker>(
                    pool_->loop(index), redisConfig, index, workerCount,
                    [this](std::string linkId, WorkerTcpRuntime::NativeSocket handle,
                           std::string remote) {
                        if (workers_.empty()) {
  ...
  private:
    ...
    std::vector<std::unique_ptr<SouthbridgeWorker>> workers_;
