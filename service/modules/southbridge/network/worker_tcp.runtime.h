...
class WorkerTcpRuntime final {
  public:
    ...
    WorkerTcpRuntime(const WorkerTcpRuntime&) = delete;
    WorkerTcpRuntime& operator=(const WorkerTcpRuntime&) = delete;

    ~WorkerTcpRuntime() { stop(); }
    ...
    void adoptServerSocket(std::string linkId, NativeSocket handle, std::string remote) {
        ...
        addConnection(definition->second, {}, bridge::nextMessageId(), std::move(remote),
                      std::move(socket));
    }
  ...
  private:
    ...
    struct ClientTarget : std::enable_shared_from_this<ClientTarget> {
        WorkerTcpRuntime& owner;
        LinkDefinition link;
        LinkTargetDefinition target;
        asio::ip::tcp::socket socket;
        WorkerTimerScheduler::Token reconnectToken = 0;
        ...
        ClientTarget(WorkerTcpRuntime& ownerValue, LinkDefinition linkValue,
        ...
        void connect() {
            ...
            socket.async_connect({address, target.port}, [self = shared_from_this()](
                ...
                self->connectionId = bridge::nextMessageId();
    ...
    void addConnection(const LinkDefinition& link, std::string targetId, std::string id,
        ...
        auto connection = std::make_shared<Connection>(
            ...
            [this, linkId = link.id](std::string_view currentId, std::string_view currentRemote,
                ...
                activity = bridge::utcNowMilliseconds();
                ...
                bridge::IngressPacket packet{.messageId = bridge::nextMessageId(),
                                             .linkId = linkId,
    ...
    void publish(std::string_view linkId, std::string state, std::string_view reason = {},
        ...
        if (!onState_)
            return;
        WorkerLinkState status{.linkId = std::string(linkId),
