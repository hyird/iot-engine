...
RUVIA_REQUEST_MODEL(LinkDebugBody, RUVIA_REQUIRED_FIELD(enabled, ruvia::Bool));

RUVIA_REQUEST_MODEL(LinkTargetBody, RUVIA_REQUIRED_FIELD(id, ruvia::String, RUVIA_MAX(64, "目标 ID 不能超过 64 个字符")), RUVIA_REQUIRED_FIELD(name, ruvia::String, RUVIA_MAX(100, "目标名称不能超过 100 个字符")), RUVIA_REQUIRED_FIELD(ip, ruvia::String, RUVIA_MAX(50, "目标 IP 不能超过 50 个字符")), RUVIA_REQUIRED_FIELD(port, ruvia::Int64, RUVIA_MIN(1, "目标端口必须在 1 - 65535 之间"), RUVIA_MAX(65535, "目标端口必须在 1 - 65535 之间")), RUVIA_OPTIONAL_FIELD(status, ruvia::String, RUVIA_ONE_OF("目标状态无效", "enabled", "disabled")));
