...
class EdgeService {
  ...
  private:
    ...
    static ruvia::Task<void> fillNode(Context& c, EdgeNodeDto& node, const Row& row) {
        ...
        if (row[51].value() || row[57].value() || row[58].value()) {
            ...
            traffic.template set<"complete">(row[55].value().value_or(std::string_view{}) == "t");
            if (row[56].value()) traffic.template set<"month">(*row[56].value());
            if (row[57].value()) traffic.template set<"monthlyUploadBytes">(*row[57].value());
            if (row[58].value()) traffic.template set<"monthlyDownloadBytes">(*row[58].value());
