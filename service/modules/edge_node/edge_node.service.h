...
class EdgeService {
  ...
  private:
    ...
    static ruvia::Task<void> fillNode(Context& c, EdgeNodeDto& node, const Row& row) {
        ...
        if (row[51].value() || row[56].value() || row[57].value()) {
        ...
        if (row[58].value() || row[63].value() || row[64].value()) {
            TcpTrafficDto vpn(ruvia::ModelOptions{ .resource = c.arena() });
            if (row[58].value()) vpn.template set<"uploadBytes">(*row[58].value());
            if (row[59].value()) vpn.template set<"downloadBytes">(*row[59].value());
            if (row[60].value()) vpn.template set<"intervalMs">(*row[60].value());
            if (row[61].value()) vpn.template set<"sampleId">(*row[61].value());
            if (row[62].value()) vpn.template set<"month">(*row[62].value());
            if (row[63].value()) vpn.template set<"monthlyUploadBytes">(*row[63].value());
            if (row[64].value()) vpn.template set<"monthlyDownloadBytes">(*row[64].value());
