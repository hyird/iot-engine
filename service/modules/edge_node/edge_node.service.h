...
class EdgeService {
  ...
  private:
    ...
    static ruvia::Task<void> fillNode(Context& c, EdgeNodeDto& node, const Row& row) {
        ...
        if (row[51].value() && row[52].value() && row[53].value() && row[54].value()) {
            TcpTrafficDto traffic(ruvia::ModelOptions{ .resource = c.arena() });
            traffic.template set<"uploadBytes">(*row[51].value());
            traffic.template set<"downloadBytes">(*row[52].value());
            traffic.template set<"intervalMs">(*row[53].value());
            traffic.template set<"sampleId">(*row[54].value());
