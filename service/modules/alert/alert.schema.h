...
class AlertIdValidator final : public ruvia::Middleware<AlertIdValidator> {
  public:
    RUVIA_VALIDATE_PARAM(AlertIdParams, RUVIA_RULE(id, RUVIA_REQUIRED("id 不能为空"), RUVIA_CUSTOM("id 必须是 UUID", service::common::isUuidField)))
};

class AlertListValidator final : public ruvia::Middleware<AlertListValidator> {
  public:
    RUVIA_VALIDATE_QUERY(AlertListQuery, RUVIA_RULE(page, RUVIA_MIN(1, "page 必须大于 0")), RUVIA_RULE_NAME("pageSize", pageSize, RUVIA_MIN(1, "pageSize 必须在 1 - 100 之间"), RUVIA_MAX(100, "pageSize 必须在 1 - 100 之间")))
};
...
class AlertPayloadValidator final {
  public:
    static ApplyTemplateInput applyTemplateInput(const ruvia::JsonValue& payload) {
  ...
  public:
    static std::vector<std::string> requiredUuids(const ruvia::JsonValue& payload, std::string_view field, std::string_view message) {
  ...
  private:
    static std::string enumString(const ruvia::JsonValue& payload, std::string_view field, std::string_view fallback, std::initializer_list<std::string_view> allowed) {
    ...
    static std::int64_t integer(const ruvia::JsonValue& payload, std::string_view field, std::int64_t fallback, std::int64_t minimum, std::int64_t maximum) {
        const auto raw = service::utils::jsonField(payload, field);
        if (!raw) {
            return fallback;
        }
    ...
    static std::string array(const ruvia::JsonValue& payload, std::string_view field, std::string_view fallback) {
    ...
    static std::string requiredArray(const ruvia::JsonValue& payload, std::string_view field, std::string_view message) {
    ...
    static std::string requiredConditions(const ruvia::JsonValue& payload, std::string_view field, std::string_view message) {
  ...
  private:
    ...
    static std::string requiredConditionString(const ruvia::JsonValue& object, std::string_view field, std::string_view message, std::size_t maximum) {
    ...
    static std::optional<std::string> optionalConditionString(const ruvia::JsonValue& object, std::string_view field, std::size_t maximum) {
    ...
    static std::optional<std::string> optionalScalarText(const ruvia::JsonValue& object, std::string_view field) {
    ...
    static std::optional<std::int64_t> optionalIntegerField(const ruvia::JsonValue& object, std::string_view field) {
...
namespace service::alert {
class AlertGroupedValidator final : public ruvia::Middleware<AlertGroupedValidator> {
  public:
    RUVIA_VALIDATE_QUERY(AlertGroupedQuery, RUVIA_RULE(days, RUVIA_MIN(1, "days 必须在 1 - 365 之间"), RUVIA_MAX(365, "days 必须在 1 - 365 之间")))
};
