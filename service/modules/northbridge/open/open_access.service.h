...
class OpenAccessService final {
  public:
    static OpenAccessService& instance() {
        static OpenAccessService service;
        return service;
    }
...
FROM device WHERE id = $1::uuid AND deleted_at IS NULL LIMIT 1)sql",
        ...
        for (const auto& point : points)
            pipeline.hgetAll(
                service::northbridge::telemetry::latest::elementKey(device[1].text(), point.id));
...
inline OpenAccessService& openAccessService() { return OpenAccessService::instance(); }
