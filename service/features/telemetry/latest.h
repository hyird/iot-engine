...
ruvia::Task<void> eraseDevice(const Redis& redis, std::string_view deviceCode) {
    co_await service::message::redis::eraseHash(redis, stateKey(deviceCode));
    co_await service::message::redis::eraseMatching(
...
inline ruvia::Task<void> hydrate(ruvia::WebWorkerContext& context) {
    ...
    co_await service::message::redis::eraseMatching(redis, "iot:latest:device:*");
    co_await service::message::redis::eraseMatching(redis, "iot:state:device:*");
