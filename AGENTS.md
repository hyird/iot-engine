...
- 当前 Ruvia `DbColumnOptions` 支持 `enumName` 与 `defaultExpression`；实体应准确记录实际使用列的 PostgreSQL 枚举类型和默认表达式。枚举仍按文本值读写，物理类型与历史默认值由 `service/config` 中的 Schema/迁移维护，不因补充实体元数据而改写历史迁移或改变真实列类型。
