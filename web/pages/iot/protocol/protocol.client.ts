...
export const getList = (params?: Protocol.Query, config?: RequestConfig) =>
    liveRead<PaginatedResult<Protocol.Item>>(appendQueryParams(BASE, params), config);
export const getAll = (params?: Protocol.Query, requestConfig?: RequestConfig) => {
    const { page: _page, pageSize: _pageSize, ...filters } = params ?? {};
    return getList({ ...filters, page: 1, pageSize: MAX_PAGE_SIZE }, requestConfig).switchMap((first) => {
        const count = Math.max(1, first.totalPages ?? Math.ceil(first.total / MAX_PAGE_SIZE));
        const pages = Array.from({ length: count }, (_, index) => index === 0
            ? LiveResource.value(first)
            : getList({ ...filters, page: index + 1, pageSize: MAX_PAGE_SIZE }, requestConfig));
        return LiveResource.combine(pages).map((results) => results.flatMap((page) => page.list));
    });
};
export const getDetail = (id: string) =>
    liveRead<Protocol.Item>(`${BASE}/${protocolIdSchema.parse(id)}`);
...
export const update = (id: string, data: Protocol.UpdateDto) =>
    request.post<void>(`${BASE}/${protocolIdSchema.parse(id)}/revisions`, protocolUpdateSchema.parse(data));
export const getRevisions = (id: string) =>
    liveRead<{ revision: number; name: string; origin: string; created_at: string }[]>(`${BASE}/${protocolIdSchema.parse(id)}/revisions`);
...
export const getOptions = (protocol: Protocol.Type) =>
    liveRead<PaginatedResult<Protocol.Option>>(
