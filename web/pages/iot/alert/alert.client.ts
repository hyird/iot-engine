...
export const getRules = (params?: Record<string, unknown>) =>
    liveRead<PaginatedResult<Alert.RuleItem>>(appendQueryParams(`${BASE}/rules`, params));
export const getRuleDetail = (id: string) => liveRead<Alert.RuleItem>(`${BASE}/rules/${id}`);
...
export const getTemplates = (params?: Record<string, unknown>) =>
    liveRead<PaginatedResult<Alert.TemplateItem>>(
        appendQueryParams(`${BASE}/templates`, params)
    );
export const getTemplateDetail = (id: string) =>
    liveRead<Alert.TemplateDetail>(`${BASE}/templates/${id}`);
...
export const getRecords = (params?: Record<string, unknown>) =>
    liveRead<PaginatedResult<Alert.RecordItem>>(appendQueryParams(`${BASE}/records`, params));
...
export const getStats = () => liveRead<Alert.ActiveStats>(`${BASE}/stats`);
export const getGrouped = (days = 7) =>
    liveRead<Alert.GroupedRecord[]>(appendQueryParams(`${BASE}/records/grouped`, { days }));

export const getDeviceOptions = () => liveRead<Device.Option[]>('/v1/device/options');
