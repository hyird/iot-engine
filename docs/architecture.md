...
边缘节点的功能对齐、nanopb 线协议、多平台来源路由和 OpenWrt 包约束见
[`edge-node.md`](edge-node.md)。边缘网关同样不得绕过本文规定的 Redis 运行时边界。
...
GB28181 是运行时可关闭的能力。`GB28181_ENABLED=false` 时不创建 GB28181 数据投影器，
不恢复 GB28181 PostgreSQL/Redis 投影，也不启动 SIP Actor、ZLMediaKit 工作线程和媒体
监听器。GB28181 业务 API 返回 404，前端菜单和直接访问页面都不展示业务内容；健康
接口保留，用于前端判定能力状态和运维诊断。ZLMediaKit C SDK 仍静态链接在同一个发布
文件中，因此其进程装载期注册不属于 GB28181 运行时启动，也不受该开关控制。
...
| Server 模式注册码/心跳（仅平台直采） | 按配置支持 | 按配置支持 | 不支持 |
| 周期主动读取 | 支持 | 支持 | 不支持 |
| 主动写入 | 支持 | 支持 | 按功能码配置 |
...
设备在线状态只有一个口径：最新有效数据时间未超过设备的 `online_timeout`。TCP
连接、边缘节点 WebSocket、监听端口和注册结果均单独展示，不得反推设备在线。注册状态
只属于平台直采 TCP Server 下配置了注册码的 DTU 透传设备；边缘采集没有设备注册状态。
