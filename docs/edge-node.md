...
边缘节点将“运行事件”“当前状态”和“任务结果”分开处理：

- 运行事件只记录生命周期变化、配置结果、链路变化和故障，不记录每个采集或遥测周期；
- 正常采集和遥测入队不写日志，当前待上报压力由心跳中的 `outbox_records`、
  `outbox_bytes` 表示；
- 网络配置、固件升级、设备命令等操作使用各自的强类型结果，并进入平台任务记录；
- 原始收发报文只在 `debug` 等级启用，默认 `info` 不输出报文流量。

因此，运行事件为空可以表示节点稳定运行，不能据此判断节点没有采集数据；采集与上报
状态应结合最后心跳、设备状态和待上报队列判断。
...
- `.proto` 是协议事实来源；平台副本位于 `service/features/edge/edge.proto`；
...
平台不接收注册 token。新节点按 IMEI 进入待审批状态，管理员批准后复用当前 WebSocket
完成注册；审批和拒绝操作必须留有审计。
...
- 节点自动生成并维护的内部连接 ID，不在 LuCI 中配置；
- HTTP/HTTPS `url` 和启用状态；当前部署明确不校验 HTTPS 服务端证书；
- `priority`；
- outbox 容量和重连策略。

平台在首次 Hello 中接受节点自动生成的连接 ID，并在该 WebSocket 会话中原样绑定；用户
无需查询或填写平台 ID。默认 ID
