...
- 按部署要求，节点使用 WSS 加密但不校验服务端证书链和主机名；该模式不能防止中间人
...
| 离线续传 | SQLite 缓存，100 条一批，ACK 后删除，超时重发 | 不使用数据库；完整 nanopb 消息按平台写入 `/tmp/iot-edge`，进程重启可恢复，设备重启/断电后清空 |
...
OpenWrt 节点 iot-edge
├── PlatformSession[platform_id]（第一阶段只启用一个，第二阶段允许多个）
...
- `.proto` 和 `.options` 是协议事实来源；
- 仓库提交生成后的 `.pb.c/.pb.h`，OpenWrt 交叉编译和设备运行时不依赖 Python、
  `protoc` 或完整 Google protobuf；
- 设备程序只编译 `pb_common.c`、`pb_encode.c`、`pb_decode.c` 和生成文件；
- CI 重新生成并比较产物，防止 `.proto` 与生成文件漂移；
- 禁止使用 protobuf map、`google.protobuf.Any`、`Struct` 或设备端无边界的动态数组。

### 4.2 传输与封装

平台连接继续使用 WSS，路径为 `/edge/v1/connect`。每个 WebSocket Binary Message 只
包含一个 nanopb `Envelope`，WebSocket 帧边界就是消息边界，不再套 JSON 或 base64。
...
`/tmp/iot-edge/<platform_id>/staging/`，每个文件直接保存一条 nanopb `ConfigBegin`、
...
OpenWrt UCI 中每个平台连接是独立 section，包含：

- 本地 `platform_id`；
- `url` 和启用状态；当前部署明确不校验 WSS 服务端证书；
- enrollment token 或已签发凭据的引用，密钥文件权限必须是 `0600`；
- `priority`；
- 是否是唯一的 `network_owner`；
- outbox 容量和重连策略。
...
网络配置只接受 UCI 中唯一 `network_owner` 平台的请求。其他平台只能读取脱敏后的接口
...
/etc/init.d/iot-edge
/etc/config/iot-edge
/etc/iot-edge/credentials/
/usr/share/iot-edge/NOTICE
```

- 使用标准 OpenWrt package `Makefile`，由 OpenWrt SDK 为目标架构生成 `.ipk`；
- 使用 procd 监督，支持 respawn、reload、资源限制和干净停止；
- 使用 UCI 配置、ubus 状态与 reload 接口；
- 动态依赖显式声明，nanopb 源码编入包内，不要求设备安装 protobuf；
- 分平台 outbox 直接写 `/tmp/iot-edge/<platform_id>/outbox/*.pb`，不写 flash；
...
- 验证 procd 状态、ubus 状态、UCI reload、WSS 加密连接和日志脱敏；
