...
平台直采的 SL651、Modbus、S7、DL/T645、三菱 MC/SLMP、欧姆龙 FINS 共用协议定义、会话创建和运行接口，并提供六个配置页面。协议对象由实际接入连接的 Collector Worker 持有，协议层不执行数据库、Redis 或网络 I/O。
...
新增协议还需要独立完成管理 API 校验、数据库配置投影、设备展示、告警要素和边缘固件接入。现有 JSON 配置、Redis 快照与 Protobuf 字段属于已部署契约，新增字段不得改变旧字段编号或语义。

## 新增工业协议

| 协议 | 平台中心采集 | EdgeNode 0.3.46 本地采集 | 配置内容 |
| --- | --- | --- | --- |
| MC/SLMP | TCP，二进制 3E / 4E | TCP Client / Server | 帧格式、路由、软元件区、地址、数值类型和字节序 |
| FINS | FINS/TCP，节点协商 | TCP Client / Server | 源/目标网络及节点、内存区、字/位地址和数值类型 |
| DL/T645 | 1997 / 2007，TCP 透明传输 | TCP 或串口 | 12 位电表地址、DI、BCD/有符号 BCD/HEX、长度、小数位及写入凭据 |

三种协议复用 `polling` 会话的轮询、超时、指令排队和写后回读流程；MC、FINS、DL/T645 目录负责各自报文。`register_value` 集中处理寄存器数值和字节序，Modbus 同样使用该实现。

每个设备类型最多 256 个点位。点位标识使用 UUID；只读点位不能下发写命令。写入成功须收到协议应答并完成回读验证；超时断开连接，避免迟到应答确认新任务。DL/T645 支持分帧续读，BCD 通过十进制文本传递以保留精度，调试发送报文隐藏写密码和操作者代码。

MC/SLMP 当前不支持 ASCII 帧，FINS 当前不支持 UDP；三种协议不提供自动点位发现。MC/FINS 按一个连接对应一个设备配置，DL/T645 按电表地址区分共享总线设备。

边缘节点通过能力报告声明协议，平台只向声明支持的节点下发新协议配置。Protobuf 以新增字段和枚举扩展，保留 0.3.44 的原有协议、任务和升级路径；旧固件需要升级到 0.3.46 才能使用新增三种协议。

数据库迁移 `0049_industrial_protocols` 扩展协议约束和电表地址绑定，保持历史迁移不变，DDL 在同一事务块执行。
...
- `tests/industrial-protocol-test.cpp`：标准报文、边界、分片、错误路由、序号、校验和、轮询和写后回读。
- `tests/industrial-protocol-integration.ts`：真实 API、配置投影、TCP 模拟设备、入库和命令回读，覆盖 MC 3E/4E、FINS、DL/T645 1997/2007。
- `tests/industrial-edge-integration.ts`：能力报告、配置快照、精确遥测入库、边缘命令派发及旧能力报告回退。
- `tests/industrial-protocol-migration-integration.ts`：新库、旧库升级、重复执行、漂移拒绝和失败回滚。
- 固件仓库的 `edge_industrial_test` 和 `edge_acquisition_test`：纯 C 编解码及五种模式的实际 TCP 读写回读。

模拟设备验证不替代真实 PLC、电表和 RS485 总线联调；固件构建成功不表示已刷入设备。
